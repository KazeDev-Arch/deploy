...
> **⚠️ ADAPTATION PROJET (Deploy) — À LIRE AVANT TOUT.** Ce skill est utilisé ici **uniquement pour le visuel et l'UX des features** (design de l'écran, UX review, accessibilité, responsive, motion) — voir « Adaptation projet » ci-dessous. Le **design system ne se redéfinit jamais** : il est déjà fixé (shadcn, monochrome noir & blanc, tokens de `src/styles.css`, cf. `context/02-design-system.md`). Ne jamais produire de palette, police ou thème alternatif.
...
## Adaptation projet (Deploy) — périmètre et limites

**Ce que le skill apporte à chaque feature :**
1. **Avant l'implémentation visuelle** : interroger la base locale (workflow ci-dessous) pour la structure de la page, les patterns UX et les anti-patterns — puis **traduire les recommandations dans le design system existant** (primitives shadcn, tokens sémantiques, typographie Manrope). Les palettes/font pairs de la base servent de raisonnement (hiérarchie, contraste, densité), **jamais de valeurs à copier**.
2. **UX review systématique** : passer la feature en revue avec la table de priorités (accessibilité, feedback, responsive, formulaires, navigation) et la checklist `references/pro-rules.md` avant de livrer.
3. **Bonnes pratiques** : anti-patterns à éviter + guidelines UX (119 règles dans `references/quick-reference.md`).

**Ce qui ne change jamais (design system verrouillé) :**
- **Aucune palette du skill ne remplace les tokens shadcn.** Pas de couleurs en dur, pas de nouvelle police (Manrope est fixée), pas de nouveau fichier de thème.
- **Stack unique : `--stack shadcn`** (dataset fourni dans `data/stacks/shadcn.csv`) — ne pas utiliser les stacks react/nextjs/html-tailwind pour le code final.
- Icônes : **`lucide-react`** uniquement (les recommandations d'icônes du skill s'appliquent au choix sémantique, pas à l'installation d'une autre lib).
- Recommandations en conflit avec le design system → **le design system gagne** (note-le dans `08-decisions-log.md` si l'écart est durable).

**Anti-hallucination (obligatoire) :**
- **Contextualise avant d'implémenter** : lire `context/02-design-system.md`, les conventions (`05-coding-conventions.md`) et les composants réellement installés (`npx shadcn@latest info`) avant toute décision visuelle. Vérifier l'existence de chaque composant/primitive utilisée dans `src/components/ui/` — ne jamais inventer une API ou un composant.
- **Ancre chaque recommandation** dans un résultat de recherche du skill, une règle du design system, ou du code existant. Si la recherche ne renvoie rien de vérifiable, le dire et appliquer les defaults (cf. « If a search returns 0 results ») — ne jamais présenter un default comme un résultat de la base.
- **Ne jamais suggérer** : une nouvelle police, une palette de couleurs, un framework CSS, une lib d'icônes, ou une refonte du thème — ces décisions sont prises (voir `02-design-system.md`).

### Chemin du script (copie locale du repo)

Ce dépôt contient une **copie locale** du skill : invoquer le script par son chemin dans le repo, pas via `${CLAUDE_PLUGIN_ROOT}` (valide uniquement pour le skill distribué en plugin) :

```bash
python ".agents/skills/ui-ux-pro-max/scripts/search.py" "<query>" --domain <domain>
```

Si `python` est introuvable, essayer `python3`, puis `py -3`. Python 3.x requis, aucune dépendance externe. Les chemins relatifs partent de la racine du repo.
...
**Dans ce projet (Deploy) :** l'appliquer à **chaque feature** dont la partie visuelle est implémentée ou revue (pages, composants, formulaires, états de chargement/vide/erreur) ; le coupler aux règles shadcn (`.agents/skills/shadcn/`) qui font foi pour le code UI. Le skip reste valable pour le pur backend (server functions, loaders, schémas, DB).
...
> **Copie locale de ce repo : utiliser `python ".agents/skills/ui-ux-pro-max/scripts/search.py" ...` (voir « Adaptation projet »).**
...
**Dans ce repo : ne PAS utiliser `--persist`** — le design system existe déjà (`context/02-design-system.md` + `src/styles.css`) et fait foi ; écrire un MASTER.md créerait une seconde source de vérité contradictoire.
...
## Workflow feature (Deploy) — du besoin au visuel livré

À exécuter pour **chaque feature avec une surface visuelle** :

1. **Contextualiser (anti-hallucination)** — lire `context/02-design-system.md` + `05-coding-conventions.md`, et vérifier les composants réellement installés (`npx shadcn@latest info`, contenu de `src/components/ui/`). Identifier les écrans/états concernés (chargement, vide, erreur).
2. **Rechercher les patterns UX** — requêtes ciblées (2–5 termes, un domaine), ex. `"reading progress article" --domain ux`, puis `"<sujet>" --stack shadcn` pour l'implémentation.
3. **Traduire dans le design system** — mapper chaque recommandation sur les primitives shadcn (`Skeleton`, `Empty`, `Badge`, `Separator`, `Field`...) et les tokens sémantiques ; ne rien introduire de neuf côté palette/police.
4. **Implémenter** — respecter les règles shadcn (`.agents/skills/shadcn/`) et les conventions (`05-coding-conventions.md`).
5. **UX review avant livraison** — repasser la table de priorités (accessibilité, feedback, responsive, formulaires, navigation) sur l'écran modifié + la checklist `references/pro-rules.md` (et `quick-reference.md` pour la règle précise).
6. **Livrer** — signals visuels couverts : loading (`Skeleton`), vide (`Empty`), erreur, paywall explicite (règle d'or du projet, cf. CLAUDE.md).

> Rappel : l'UX review du skill **complète** la revue de code (`review`) ; les règles shadcn et le design system priment en cas de conflit.
